...
CLASS lcl_test DEFINITION
  ...
  PRIVATE SECTION.
    ...
    METHODS get_messages
      IMPORTING log_handle  TYPE balloghndl
      EXPORTING texts       TYPE table_of_strings
    ...
    METHODS auto_saves_named_log           FOR TESTING.
    METHODS auto_saves_reopened_log        FOR TESTING.
    METHODS can_log_string                 FOR TESTING.
    METHODS can_log_char                   FOR TESTING.
    METHODS can_log_symsg                  FOR TESTING.
    METHODS can_log_bapiret1               FOR TESTING.
    METHODS can_log_bapiret2               FOR TESTING.
    METHODS can_log_bapi_coru_return       FOR TESTING.
    METHODS can_log_bapi_order_return      FOR TESTING.
    METHODS can_log_rcomp                  FOR TESTING.
    METHODS can_log_prott                  FOR TESTING.
    METHODS can_log_sprot                  FOR TESTING.
    METHODS can_log_bal_s_msg              FOR TESTING.
    METHODS can_log_bapirettab             FOR TESTING.
    METHODS can_log_err                    FOR TESTING.
    METHODS can_log_chained_exceptions     FOR TESTING.
    METHODS can_log_batch_msgs             FOR TESTING.
    METHODS can_log_any_simple_structure   FOR TESTING.
    METHODS can_log_any_deep_structure     FOR TESTING.
    METHODS can_log_loggable_object        FOR TESTING.
    METHODS can_add_msg_context            FOR TESTING.
    METHODS can_add_callback_sub           FOR TESTING.
    METHODS can_add_callback_fm            FOR TESTING.
    METHODS can_use_and_chain_aliases      FOR TESTING.
    METHODS return_proper_status           FOR TESTING.
    METHODS return_proper_length           FOR TESTING.
    METHODS can_add_table_msg_context      FOR TESTING RAISING cx_static_check.
    METHODS can_log_string_and_export      FOR TESTING.
    METHODS can_log_exception_and_export   FOR TESTING.
    METHODS can_change_description         FOR TESTING RAISING cx_static_check.
    METHODS can_log_callback_params        FOR TESTING RAISING cx_static_check.
    METHODS can_log_log                    FOR TESTING.
    METHODS can_log_bapi_alm_return        FOR TESTING.
    METHODS can_log_bapi_meth_message      FOR TESTING RAISING cx_static_check.
    METHODS can_log_bapi_status_result     FOR TESTING RAISING cx_static_check.
    METHODS can_log_detlevel               FOR TESTING RAISING cx_static_check.
    METHODS can_log_exception_with_context FOR TESTING RAISING cx_static_check.
    METHODS nothing_logged_for_empty_obj   FOR TESTING RAISING cx_static_check.
...
CLASS lcl_test IMPLEMENTATION.
  ...
  METHOD can_create_anon_log.
    cl_abap_unit_assert=>assert_bound( act = anon_log
                                       msg = 'Cannot Instantiate Anonymous Log' ).
  ENDMETHOD.

  METHOD can_create_named_log.
    cl_abap_unit_assert=>assert_bound( act = named_log
  ...
  METHOD can_create_expiring_log_days.
    ...
    cl_abap_unit_assert=>assert_bound( act = expiring_log
                                       msg = 'Cannot Instantiate Expiring Log' ).

    CALL FUNCTION 'BAL_LOG_HDR_READ'
      EXPORTING i_log_handle = expiring_log->handle
      IMPORTING e_s_log      = act_header.
    ...
    cl_abap_unit_assert=>assert_true( act = act_header-del_before
  ...
  METHOD can_create_expiring_log_date.
    ...
    cl_abap_unit_assert=>assert_equals( exp = lv_expire
                                        act = act_header-aldate_del
  ...
  METHOD can_add_log_context.
    ...
    cl_abap_unit_assert=>assert_equals( exp = 'T005T'
                                        act = act_header-context-tabname
                                        msg = 'Did not add context to log' ).
    cl_abap_unit_assert=>assert_equals( exp = random_country_data
  ...
  METHOD auto_saves_named_log.
    ...
    CALL FUNCTION 'BAL_DB_LOAD'
      EXPORTING i_t_lognumber = log_numbers.

    cl_abap_unit_assert=>assert_equals( exp = msg
                                        act = get_first_message( named_log->handle )
                                        msg = 'Did not write to named log' ).
  ENDMETHOD.
  ...
  METHOD auto_saves_reopened_log.
    ...
    cl_abap_unit_assert=>assert_equals( exp = 'This message is in the database'
                                        act = act_text
                                        msg = 'Did not autosave to reopened log' ).

    READ TABLE act_texts INDEX 2 INTO act_text.
    cl_abap_unit_assert=>assert_equals( exp = 'This is another message in the database'
                                        act = act_text
                                        msg = 'Did not autosave to reopened log' ).
  ENDMETHOD.
  ...
  METHOD can_log_symsg.
    ...
    DATA expected_details TYPE bal_s_msg.
    DATA actual_details   TYPE bal_s_msg.
    DATA actual_text      TYPE char200.

    symsg-msgty = 'W'.
    expected_details-msgty = symsg-msgty.
    symsg-msgid = 'BL'.
    expected_details-msgid = symsg-msgid.
    symsg-msgno = '001'.
    expected_details-msgno = symsg-msgno.
    symsg-msgv1 = 'This'.
    expected_details-msgv1 = symsg-msgv1.
    symsg-msgv2 = 'is'.
    expected_details-msgv2 = symsg-msgv2.
    symsg-msgv3 = 'a'.
    expected_details-msgv3 = symsg-msgv3.
    symsg-msgv4 = 'test'.
    ...
    CALL FUNCTION 'BAL_LOG_MSG_READ'
      ...
      IMPORTING e_s_msg        = actual_details
                e_txt_msg      = actual_text.

    cl_abap_unit_assert=>assert_not_initial( act = actual_details-time_stmp
                                             msg = 'Did not log system message properly' ).

    expected_details-msg_count = 1.
    CLEAR actual_details-time_stmp.

    cl_abap_unit_assert=>assert_equals( exp = expected_details
                                        act = actual_details
                                        msg = 'Did not log system message properly' ).
    cl_abap_unit_assert=>assert_equals( exp = 'This is a test'
                                        act = condense( actual_text )
                                        msg = 'Did not log system message properly' ).
    cl_abap_unit_assert=>assert_true( act = anon_log->zif_logger~has_warnings( )
  ...
  METHOD can_log_bapiret1.
    ...
    bapi_msg-type = 'W'.
    expected_details-msgty = bapi_msg-type.
    bapi_msg-id = 'BL'.
    expected_details-msgid = bapi_msg-id.
    bapi_msg-number = '001'.
    expected_details-msgno = bapi_msg-number.
    bapi_msg-message_v1 = 'This'.
    expected_details-msgv1 = bapi_msg-message_v1.
    bapi_msg-message_v2 = 'is'.
    expected_details-msgv2 = bapi_msg-message_v2.
    bapi_msg-message_v3 = 'a'.
    expected_details-msgv3 = bapi_msg-message_v3.
  ...
  METHOD can_log_bapi_order_return.
    ...
    bapi_return_temp-type = 'E'.
    expected_details-msgty = bapi_return_temp-type.
    bapi_return_temp-id = 'BL'.
    expected_details-msgid = bapi_return_temp-id.
    bapi_return_temp-number = '001'.
    expected_details-msgno = bapi_return_temp-number.
    bapi_return_temp-message_v1 = 'This'.
    expected_details-msgv1 = bapi_return_temp-message_v1.
    bapi_return_temp-message_v2 = 'is'.
    expected_details-msgv2 = bapi_return_temp-message_v2.
    bapi_return_temp-message_v3 = 'a'.
    expected_details-msgv3 = bapi_return_temp-message_v3.
    ...
    cl_abap_unit_assert=>assert_true( act = anon_log->zif_logger~has_errors( )
                                      msg = 'Did not log or fetch system message properly' ).
  ENDMETHOD.
  ...
  METHOD can_log_sprot.
    ...
    sprot_msg-severity = 'W'.
    expected_details-msgty = sprot_msg-severity.
    sprot_msg-ag = 'BL'.
    expected_details-msgid = sprot_msg-ag.
    sprot_msg-msgnr = '001'.
    expected_details-msgno = sprot_msg-msgnr.
    sprot_msg-var1 = 'This'.
    expected_details-msgv1 = sprot_msg-var1.
    sprot_msg-var2 = 'is'.
    expected_details-msgv2 = sprot_msg-var2.
    sprot_msg-var3 = 'a'.
    expected_details-msgv3 = sprot_msg-var3.
    sprot_msg-var4 = 'test'.
  ...
  METHOD can_log_bapirettab.
    ...
    DATA bapi_msg      TYPE bapiret2.
    DATA exp_texts     TYPE table_of_strings.
    DATA exp_text      TYPE string.
    DATA exp_details   TYPE ty_bal_tt_msg.
    DATA exp_detail    TYPE bal_s_msg.
    DATA act_texts     TYPE table_of_strings.
    DATA act_text      TYPE string.
    DATA act_details   TYPE ty_bal_tt_msg.
    DATA act_detail    TYPE bal_s_msg.

    DEFINE bapiret_messages_are.
      bapi_msg-type = &1.
      exp_detail-msgty = bapi_msg-type.
      bapi_msg-id = &2.
      exp_detail-msgid = bapi_msg-id.
      bapi_msg-number = &3.
      exp_detail-msgno = bapi_msg-number.
      bapi_msg-message_v1 = &4.
      exp_detail-msgv1 = bapi_msg-message_v1.
      bapi_msg-message_v2 = &5.
      exp_detail-msgv2 = bapi_msg-message_v2.
      bapi_msg-message_v3 = &6.
      exp_detail-msgv3 = bapi_msg-message_v3.
      bapi_msg-message_v4 = &7.
      exp_detail-msgv4 = bapi_msg-message_v4.
    ...
    DO 3 TIMES.
      ...
      cl_abap_unit_assert=>assert_not_initial( act = act_detail-time_stmp
                                               msg = 'Did not log system message properly' ).

      exp_detail-msg_count = 1.
      CLEAR act_detail-time_stmp.

      cl_abap_unit_assert=>assert_equals( exp = exp_detail
                                          act = act_detail
      ...
      cl_abap_unit_assert=>assert_equals( exp = exp_text
  ...
  METHOD can_log_err.
    ...
    DATA err            TYPE REF TO cx_sy_zerodivide.
    DATA act_txt        TYPE char255.
    DATA exp_txt        TYPE char255.
    " TODO: variable is assigned but never used (ABAP cleaner)
    ...
    CALL FUNCTION 'BAL_LOG_EXCEPTION_READ'
      EXPORTING i_s_msg_handle = msg_handle
      IMPORTING e_txt_msg      = act_txt.

    cl_abap_unit_assert=>assert_equals( exp = exp_txt "'Division by zero'
                                        act = act_txt
  ...
  METHOD can_log_chained_exceptions.
    ...
    DATA previous_exception TYPE REF TO lcx_t100.
    DATA caught_exception   TYPE REF TO lcx_t100.
    DATA msg_count          TYPE i.
    DATA bal_msgs           TYPE ty_bal_tt_msg.
    DATA bal_msg            TYPE bal_s_msg.

    DEFINE exceptions_are.
      CREATE OBJECT main_exception
        EXPORTING previous = previous_exception
                  id       = &1
                  no       = &2
                  msgv1    = &3
                  msgv2    = &4
    ...
    msg_count = lines( bal_msgs ).
    cl_abap_unit_assert=>assert_equals( exp = 3
                                        act = msg_count
                                        msg = c_chained_exception ).

    READ TABLE bal_msgs INDEX 1 INTO bal_msg.
    cl_abap_unit_assert=>assert_equals( exp = 'SABP_UNIT010'                                 " 'Message 1'
                                        act = bal_msg-msgid && bal_msg-msgno
                                        msg = c_chained_exception ).

    READ TABLE bal_msgs INDEX 2 INTO bal_msg.
    cl_abap_unit_assert=>assert_equals( exp = 'SABP_UNIT030'                                 " 'Message 2'
                                        act = bal_msg-msgid && bal_msg-msgno
                                        msg = c_chained_exception ).
  ...
  METHOD can_log_batch_msgs.
    ...
    DATA batch_msg  TYPE bdcmsgcoll.
    DATA bal_msgs   TYPE ty_bal_tt_msg.
    DATA bal_msg    TYPE bal_s_msg.
    DATA msg_count  TYPE i.
  ...
  METHOD can_log_any_simple_structure.
    ...
    DATA act_table   TYPE table_of_strings.
    DATA exp_table   TYPE table_of_strings.
    DATA exp_line    LIKE LINE OF exp_table.
    DATA msg_details TYPE ty_bal_tt_msg.

    struct-comp1 = 'Demo'.
    struct-comp2 = 5.
    anon_log->zif_logger~error( struct ).
    ...
    cl_abap_unit_assert=>assert_equals( exp = exp_table
                                        act = act_table
  ...
  METHOD can_add_msg_context.
    ...
    CALL FUNCTION 'BAL_LOG_MSG_READ'
      EXPORTING i_s_msg_handle = msg_handle
      IMPORTING e_s_msg        = act_details.

    cl_abap_unit_assert=>assert_equals( exp = addl_context
                                        act = act_details-context-value
                                        msg = 'Did not add context correctly' ).
    cl_abap_unit_assert=>assert_equals( exp = 'BEZEI20'
  ...
  METHOD can_add_table_msg_context.
    ...
    cl_abap_unit_assert=>assert_initial( act = act_details-context-value
                                         msg = 'Context should only be added to first line'  ).
    cl_abap_unit_assert=>assert_initial( act = act_details-context-tabname
                                         msg = 'Context should only be added to first line'  ).
  ENDMETHOD.
  ...
  METHOD can_use_and_chain_aliases.
    ...
    anon_log->zif_logger~abend( 'Severe Abort Error!' )->error( |Here's an error!| ).
    anon_log->zif_logger~warning( 'This is a warning' )->info( `Helpful Information` ).
    anon_log->zif_logger~success( |GreatSuccess| ).
    ...
    cl_abap_unit_assert=>assert_equals( exp = 'Severe Abort Error!'
                                        act = text
                                        msg = 'Didn''t log by alias' ).
    ...
    cl_abap_unit_assert=>assert_equals( exp = 'E'
                                        act = msg_detail-msgty
                                        msg = 'Didn''t log by alias' ).
    cl_abap_unit_assert=>assert_equals( exp = 'Here''s an error!'
                                        act = text
                                        msg = 'Didn''t log by alias' ).
    ...
    cl_abap_unit_assert=>assert_equals( exp = 'W'
                                        act = msg_detail-msgty
                                        msg = 'Didn''t log by alias' ).
    cl_abap_unit_assert=>assert_equals( exp = 'This is a warning'
                                        act = text
                                        msg = 'Didn''t log by alias' ).
    ...
    cl_abap_unit_assert=>assert_equals( exp = 'I'
                                        act = msg_detail-msgty
                                        msg = 'Didn''t log by alias' ).
    cl_abap_unit_assert=>assert_equals( exp = 'Helpful Information'
                                        act = text
                                        msg = 'Didn''t log by alias' ).
    ...
    cl_abap_unit_assert=>assert_equals( exp = 'S'
                                        act = msg_detail-msgty
                                        msg = 'Didn''t log by alias' ).
    cl_abap_unit_assert=>assert_equals( exp = 'GreatSuccess'
                                        act = text
                                        msg = 'Didn''t log by alias' ).
  ENDMETHOD.
  ...
  METHOD get_messages.
    ...
    DATA msg_handle      TYPE balmsghndl.
    DATA msg_detail      TYPE bal_s_msg.
    DATA exc_detail      TYPE bal_s_excr.
    DATA msg_text        TYPE char255.

    INSERT log_handle INTO TABLE handle_as_table.
    CALL FUNCTION 'BAL_GLB_SEARCH_MSG'
      EXPORTING i_t_log_handle = handle_as_table
      IMPORTING e_t_msg_handle = message_handles.
  ...
  METHOD return_proper_status.
    cl_abap_unit_assert=>assert_not_initial( act = anon_log->zif_logger~is_empty( )
                                             msg = 'Not empty at start' ).

    anon_log->zif_logger~success( 'success' ).
    anon_log->zif_logger~info( 'info' ).

    cl_abap_unit_assert=>assert_initial( act = anon_log->zif_logger~is_empty( )
                                         msg = 'Empty after add' ).
    cl_abap_unit_assert=>assert_initial( act = anon_log->zif_logger~has_errors( )
                                         msg = 'Has errors when there were no errors' ).
    cl_abap_unit_assert=>assert_initial( act = anon_log->zif_logger~has_warnings( )
                                         msg = 'Has warnings when there were no warnings' ).

    anon_log->zif_logger~error( 'error' ).
    anon_log->zif_logger~warning( 'warning' ).

    cl_abap_unit_assert=>assert_not_initial( act = anon_log->zif_logger~has_errors( )
                                             msg = 'Has no errors when there were errors' ).
    cl_abap_unit_assert=>assert_not_initial( act = anon_log->zif_logger~has_warnings( )
  ...
  METHOD can_log_string_and_export.
    DATA stringmessage TYPE string VALUE `Logging a string, guys!`.
    DATA table         TYPE bapirettab.

    anon_log->zif_logger~add( stringmessage ).

    table = anon_log->zif_logger~export_to_table( ).
  ...
  METHOD can_log_exception_and_export.
    DATA table TYPE bapirettab.
    DATA error TYPE REF TO cx_sy_zerodivide.

    TRY.
        RAISE EXCEPTION TYPE cx_sy_zerodivide.
      CATCH cx_sy_zerodivide INTO error.
        anon_log->zif_logger~add( error ).
    ENDTRY.
  ...
  METHOD can_log_callback_params.
    DATA callback_parameters TYPE bal_t_par.
    DATA parameter           LIKE LINE OF callback_parameters.
    DATA act_details         TYPE ty_bal_tt_msg.
    ...
    anon_log->zif_logger~abend( obj_to_log          = |Test W|
                                callback_fm         = 'DUMMY'
                                callback_parameters = callback_parameters ).

    anon_log->zif_logger~error( obj_to_log          = |Test E|
                                callback_fm         = 'DUMMY'
                                callback_parameters = callback_parameters ).

    anon_log->zif_logger~info( obj_to_log          = |Test I|
                               callback_fm         = 'DUMMY'
                               callback_parameters = callback_parameters ).

    anon_log->zif_logger~success( obj_to_log          = |Test S|
                                  callback_fm         = 'DUMMY'
                                  callback_parameters = callback_parameters ).

    anon_log->zif_logger~warning( obj_to_log          = |Test W|
                                  callback_form       = 'DUMMY_FORM'
                                  callback_prog       = 'DUMMY_PROG'
                                  callback_parameters = callback_parameters ).

    get_messages( EXPORTING log_handle  = anon_log->handle
                  IMPORTING msg_details = act_details ).

    LOOP AT act_details ASSIGNING <detail>.

      cl_abap_unit_assert=>assert_equals( exp = callback_parameters
  ...
  METHOD can_log_log.
    ...
    DATA message_s   TYPE string.
    DATA message_w   TYPE string.
    DATA message_e   TYPE string.
    DATA message_a   TYPE string.
    DATA texts       TYPE table_of_strings.
    DATA text        TYPE string.
    ...
    anon_log->zif_logger~info( obj_to_log = message_i ).
    anon_log->zif_logger~success( obj_to_log = message_s ).
    anon_log->zif_logger~warning( obj_to_log = message_w ).
    anon_log->zif_logger~error( obj_to_log = message_e ).
    anon_log->zif_logger~abend( obj_to_log = message_a ).

    " when
    named_log->zif_logger~add( obj_to_log = anon_log ).
    ...
    READ TABLE texts INDEX 1 INTO text.
    READ TABLE msg_details INDEX 1 INTO msg_detail.
    cl_abap_unit_assert=>assert_equals( exp = message_i
                                        act = text
    ...
    READ TABLE texts INDEX 2 INTO text.
    READ TABLE msg_details INDEX 2 INTO msg_detail.
    cl_abap_unit_assert=>assert_equals( exp = message_s
                                        act = text
                                        msg = 'Did not add logger message correctly' ).
    cl_abap_unit_assert=>assert_equals( exp = 'S'
                                        act = msg_detail-msgty
    ...
    READ TABLE texts INDEX 3 INTO text.
    READ TABLE msg_details INDEX 3 INTO msg_detail.
    cl_abap_unit_assert=>assert_equals( exp = message_w
                                        act = text
                                        msg = 'Did not add logger message correctly' ).
    cl_abap_unit_assert=>assert_equals( exp = 'W'
                                        act = msg_detail-msgty
    ...
    READ TABLE texts INDEX 4 INTO text.
    READ TABLE msg_details INDEX 4 INTO msg_detail.
    cl_abap_unit_assert=>assert_equals( exp = message_e
                                        act = text
                                        msg = 'Did not add logger message correctly' ).
    cl_abap_unit_assert=>assert_equals( exp = 'E'
                                        act = msg_detail-msgty
    ...
    READ TABLE texts INDEX 5 INTO text.
    READ TABLE msg_details INDEX 5 INTO msg_detail.
    cl_abap_unit_assert=>assert_equals( exp = message_a
                                        act = text
                                        msg = 'Did not add logger message correctly' ).
    cl_abap_unit_assert=>assert_equals( exp = 'A'
                                        act = msg_detail-msgty
  ...
  METHOD can_log_bapi_alm_return.
    ...
    bapi_alm_return_temp-type = 'E'.
    expected_details-msgty = bapi_alm_return_temp-type.
    bapi_alm_return_temp-message_id = 'BL'.
    expected_details-msgid = bapi_alm_return_temp-message_id.
    bapi_alm_return_temp-message_number = '001'.
    expected_details-msgno = bapi_alm_return_temp-message_number.
    bapi_alm_return_temp-message_v1 = 'This'.
    expected_details-msgv1 = bapi_alm_return_temp-message_v1.
    bapi_alm_return_temp-message_v2 = 'is'.
    expected_details-msgv2 = bapi_alm_return_temp-message_v2.
    bapi_alm_return_temp-message_v3 = 'a'.
    expected_details-msgv3 = bapi_alm_return_temp-message_v3.
    bapi_alm_return_temp-message_v4 = 'test'.
    expected_details-msgv4 = bapi_alm_return_temp-message_v4.
    MOVE-CORRESPONDING bapi_alm_return_temp TO <bapi_alm_return_structure>.
    anon_log->zif_logger~add( <bapi_alm_return_structure> ).
  ...
  METHOD can_log_bapi_meth_message.
    ...
    bapi_meth_message_temp-message_type = 'E'.
    expected_details-msgty = bapi_meth_message_temp-message_type.
    bapi_meth_message_temp-message_id = 'CJ'.
    expected_details-msgid = bapi_meth_message_temp-message_id.
    bapi_meth_message_temp-message_number = '036'.
    expected_details-msgno = bapi_meth_message_temp-message_number.
    MOVE-CORRESPONDING bapi_meth_message_temp TO <bapi_meth_message_structure>.
    anon_log->zif_logger~add( <bapi_meth_message_structure> ).
  ...
  METHOD can_log_bapi_status_result.
    ...
    bapi_status_result_temp-message_type = 'E'.
    expected_details-msgty = bapi_status_result_temp-message_type.
    bapi_status_result_temp-message_id = 'CNIF_STATUS'.
    expected_details-msgid = bapi_status_result_temp-message_id.
    bapi_status_result_temp-message_number = '005'.
    expected_details-msgno = bapi_status_result_temp-message_number.
    MOVE-CORRESPONDING bapi_status_result_temp TO <bapi_status_result_structure>.
    anon_log->zif_logger~add( <bapi_status_result_structure> ).
  ...
  METHOD can_log_detlevel.
    ...
    anon_log->zif_logger~success( obj_to_log = 'success'
                                  detlevel   = '1' ).
    anon_log->zif_logger~info( obj_to_log = 'info'
                               detlevel   = '2' ).
    anon_log->zif_logger~error( obj_to_log = 'error'
                                detlevel   = '3' ).
    anon_log->zif_logger~warning( obj_to_log = 'warning'
                                  detlevel   = '4' ).
    anon_log->zif_logger~abend( obj_to_log = 'abort'
                                detlevel   = '5' ).

    MESSAGE w001(bl) WITH 'This' 'is' 'a' 'test' INTO dummy.

    anon_log->zif_logger~info( detlevel = '6' ).
    ...
    anon_log->zif_logger~add( obj_to_log = bapi_msg
                              detlevel   = '7' ).

    INSERT bapi_msg INTO TABLE bapi_messages.

    anon_log->zif_logger~add( obj_to_log = bapi_messages
                              detlevel   = '8' ).

    TRY.
        RAISE EXCEPTION TYPE cx_sy_zerodivide.
      CATCH cx_sy_zerodivide INTO error.
        anon_log->zif_logger~info( obj_to_log = error
                                   detlevel   = '9' ).
    ENDTRY.
  ...
  METHOD can_log_exception_with_context.
    ...
    DATA expected_details TYPE bal_s_msg.
    DATA exp_callback     TYPE bal_s_clbk.
    DATA addl_context     TYPE bezei20 VALUE 'Berlin'.
    ...
    " When
    anon_log->zif_logger~add( obj_to_log    = exception
                              callback_form = 'FORM'
                              callback_prog = 'PROGRAM'
                              context       = addl_context ).
    ...
    CLEAR: actual_details-msg_count,
           actual_details-time_stmp,
           actual_details-context,
           actual_details-params.
  ...
  METHOD nothing_logged_for_empty_obj.
    ...
    anon_log->zif_logger~add( bapi_msgs ).
    cl_abap_unit_assert=>assert_equals( exp = 0
                                        act = anon_log->zif_logger~length( ) ).
